...
def ftmsql(request):
    ...
    startdatum = data[5]
    einddatum = data[6]
    bergingscoefficient = data[7]
    drainweerstand = data[8]
    qbot =data[9]
    ontwateringsbasis = data[10]
    nummer_meteostation=data[11]
    naam_meteostation = data[12]
    return render_to_response("ftm/grafiek.html", { 'x':x, 'y':y, 'plotje': plotje, 'ghg':ghg, 'glg':glg, 'gvg':gvg, 'gt' :gt, 'startdatum':startdatum, 'einddatum':einddatum,'bergingscoefficient': bergingscoefficient, 'drainweerstand':drainweerstand, 'qbot':qbot, 'ontwateringsbasis':ontwateringsbasis, 'nummer_meteostation':nummer_meteostation, 'naam_meteostation':naam_meteostation})
