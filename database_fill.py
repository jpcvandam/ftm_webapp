...
def meteo_in_database(nummer_meteostation, con, yyyymmdd, dr, rh, rhx, ev24):
    with con:
		...
		cur.execute("CREATE TABLE station240 (yyyymmdd TEXT, dr INT, rh INT, rhx INT, ev24 INT)")
		for i in range(1, len(yyyymmdd)):
			...
			with con:
				cur = con.cursor()
				cur.executemany("INSERT INTO station240 VALUES(?, ?, ?, ?, ?)", (meteodata,))
			#print meteodata
			#con.close()
	
###############################################################################	
#variabelen
nummer_meteostation=240
...
#    cur = con.cursor()    
#    cur.execute("SELECT * FROM station240")
...
for j in [215, 225, 235, 240, 242, 249, 251, 257, 260, 265, 267, 269, 270, 273, 275, 277, 278, 279, 280, 283, 286, 290, 310, 323,  319, 330, 340, 344, 348, 350, 356, 370, 375, 377, 380, 391,  ]:
	yyyymmdd =  meteo_lezen(j)[0]
	dr = meteo_lezen(j)[1]
	rh = meteo_lezen(j)[2]
	rhx = meteo_lezen(j)[3]
	ev24 = meteo_lezen(j)[4]
