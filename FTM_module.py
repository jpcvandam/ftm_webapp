...
def maak_plotje_aangepast(x2, y2, startdatum, einddatum, resultaat, berg, drain, qbot, ontwbas):
    ...
    array_bergingscoefficient = np.array([float(berg)]) #np.array([float(raster_q(bestandspad + "bergcoef-nzv.tif", x, y))])
    array_drainweerstand = np.array([float(drain)]) #np.array([float(raster_q(bestandspad + "drainw-nzv.tif", x, y))])
    array_qbot = np.array([float(qbot)]) #np.array([float(raster_q(bestandspad + "kwel-nzv.tif", x, y))])
    array_hgem = np.array([float(ontwbas)]) #np.array([float(raster_q(bestandspad + "ontwbas-nzv.tif", x, y))*-1.0])
